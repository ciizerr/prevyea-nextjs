...
import { CheckCircle2, XCircle, ExternalLink, Clock, ShieldQuestion, Bug } from "lucide-react";
...
export default async function AdminVerificationPage() {
    ...
    // 4. Fetch PENDING Bug Reports (Only for ADMIN and MODERATOR)
                            ...
                            <div key={pyq.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                                ...
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="text-sm text-zinc-500 mt-2">
                                        ...
                                        on {(() => {
                                            const raw = pyq.createdAt;
                                            if (!raw) return "Unknown date";
                                            let d: Date;
                                            if (raw instanceof Date) d = raw;
                                            else if (typeof raw === "string") d = new Date((raw as string).replace(" ", "T") + ((raw as string).includes("Z") ? "" : "Z"));
                                            else d = new Date(Number(raw) * 1000);
                                            return isNaN(d.getTime()) ? "Unknown date" : d.toLocaleDateString("en-US", { year: "numeric", month: "short", day: "2-digit" });
                                        })()}
                                    </div>
                                </div>
