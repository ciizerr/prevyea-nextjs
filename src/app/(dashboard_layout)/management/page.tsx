...
import { Building2, GraduationCap, BookOpen, Plus, Trash2 } from "lucide-react";
...
export default async function ManagementPage() {
    ...
    // 2. Fetch existing data for preview
        ...
        <div className="max-w-6xl mx-auto p-6 md:p-12 animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-12">
            ...
            <div>
                ...
                <p className="text-zinc-500 dark:text-zinc-400">
                    Admin portal to create and manage the application&apos;s underlying structural data such as Colleges, Courses, and Subjects.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-sm">
                    ...
                    <form action={async (formData) => { "use server"; await createCollegeAction(formData); return; }} className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-indigo-500/50 outline-none"
                            />
                        </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-800/60">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {allColleges.map(c => (
                                <span key={c.id} className="text-xs bg-zinc-100 dark:bg-zinc-800 px-3 py-1 rounded-lg text-zinc-700 dark:text-zinc-300 flex items-center gap-2">
                                    {c.name}
                                    <form action={async () => { "use server"; await deleteCollegeAction(c.id); }} className="inline">
                                        <button type="submit" title="Delete College" className="text-zinc-400 hover:text-red-500 transition-colors">
                ...
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-sm">
                    ...
                    <form action={async (formData) => { "use server"; await createCourseAction(formData); return; }} className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-emerald-500/50 outline-none"
                            />
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {allCourses.map(c => {
                                const mappedCols = allCollegeCourses.filter(cc => cc.courseId === c.id)
                                    ...
                                    .filter(Boolean).join(", ");
                                return (
                                    <span key={c.id} className="text-xs bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-200/50 dark:border-emerald-900/30 px-3 py-2 rounded-lg flex flex-col items-start gap-1.5 shadow-sm relative pr-8">
                                        ...
                                        <form action={async () => { "use server"; await deleteCourseAction(c.id); }} className="absolute top-2 right-2">
                                            <button type="submit" title="Delete Course" className="text-emerald-400 hover:text-red-500 transition-colors">
                            ...
                            {allSubjects.map(s => {
                                ...
                                return (
                                    <span key={s.id} className="text-xs bg-amber-50 text-amber-800 dark:bg-amber-500/10 dark:text-amber-400 border border-amber-200/50 dark:border-amber-900/30 px-3 py-1.5 rounded-lg flex items-center gap-1.5 relative pr-8">
                                        ...
                                        <form action={async () => { "use server"; await deleteSubjectAction(s.id); }} className="absolute top-1/2 -translate-y-1/2 right-2">
                                            <button type="submit" title="Delete Subject" className="text-amber-400 hover:text-red-500 transition-colors">
                                                <Trash2 className="h-3 w-3" />
                                            </button>
                                        </form>
                                    </span>
